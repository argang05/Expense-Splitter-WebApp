...
const NonFoodBillImageGalleryComponent = ({ tripId }) => {
  ...
  useEffect(() => {
    const fetchImageUrls = async () => {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_BASE_URL}/api/bills/non-food-bill-image-url/${tripId}`
        );
        if (response.status === 200) {
          setImageUrls(response.data); // Update state only once
        }
      } catch (err) {
        console.error(
    ...
    fetchImageUrls();
  }, [tripId]); // Only run this effect when tripId changes
...
export default NonFoodBillImageGalleryComponent;
