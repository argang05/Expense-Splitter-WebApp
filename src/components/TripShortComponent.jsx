...
const TripShortComponent = ({trip}) => {
  return (
      <NavLink to={`/trip/${trip.id}`}
          ...
          <h2 className='text-xl font-semibold'>Purpose: {trip.tripPurpose}</h2>
          <h2 className='text-xl font-semibold'>Type: {trip.tripType}</h2>
          <h2 className='text-xl font-semibold'>Country: {trip.country}</h2>
          <h2 className='text-xl font-semibold'>Duration: {trip.numberOfDays} days</h2>
      </NavLink>
  )
}
